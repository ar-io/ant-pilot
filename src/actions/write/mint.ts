...
export const mint = async (state, { input, caller }) => {
  ...
  const token =
    supportedTokens[type === PAYMENT_TYPES.ARWEAVE ? 'AR' : tokenId];
  let qty = 0;
  ...
  if (qty <= 0 || !Number.isInteger(qty)) {
    throw new ContractError(
      `Invalid payment amount: qty is less than or equal to 0, recieved ${qty} on type ${type}`,
    );
  }
  state.balances[caller] = +qty * token.conversionRate;
  state.supportedTokens[
    type === PAYMENT_TYPES.ARWEAVE ? 'AR' : tokenId
  ].revenue += qty;
  state.totalSupply = Object.values(state.balances).reduce(
