...
export async function validateExternalTokenTransfer({
  tags,
  tokenId,
  config,
  owner,
  ...
  try {
    ...
    if (!input[quantityArg] || input[quantityArg] <= 0) {
      throw new Error(
        `"${quantityArg}" is required and must be greater than 0`,
      );
    }
    ...
    if (input.function !== transferFunction) {
      // TODO: add support for other transfer function eg transferFrom
      throw new Error(`function must be "${transferFunction}"`);
    }
    const { state, validity, errorMessages } =
      await SmartWeave.contracts.readContractState(tokenId, true);

    if (!validity[SmartWeave.transaction.id]) {
      throw new Error(
        `Token transfer is not valid [Error: ${
          errorMessages[SmartWeave.transaction.id]
        }]`,
      );
    }
    if (!state.balances) {
      throw new Error(
        'Token state must have balances - the contract is not a valid token contract',
      );
    }
    if (!state.balances[owner]) {
      throw new Error(
        'Transfer was valid but owner does not have a balance for this token',
      );
    }
...
export const validateDirectRecordManagement = (state, caller, subDomain) => {
  ...
  if (!reservedRegex.test(subDomain)) {
    throw new ContractError(
      'Subdomains must be reserved before they are directly managed',
    );
  }

  if (state.records[subDomain] && state.records[subDomain].contractTxId) {
    throw new ContractError(
      'Subdomains that are owned by a contract cannot be directly managed',
    );
  }

  if (!state.controllers.includes(caller) && state.owner !== caller) {
    throw new ContractError(
      'Caller must be owner or controller for direct record management',
    );
  }
};
