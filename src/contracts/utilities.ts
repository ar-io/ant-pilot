...
export function isArweaveBase64URL(base64URL: string) {
  const trimmedbase64URL = base64URL.toString().trim();
  if (!/[a-z0-9_-]{43}/i.test(trimmedbase64URL)) {
    throw new ContractError("Invalid Arweave Base64URL.");
  }
  return trimmedbase64URL;
}
